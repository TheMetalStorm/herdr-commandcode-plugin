#!/bin/sh
# Shared helpers for the Command Code herdr plugin.
...
# Seed the cmd agent-detection override into herdr's config dir (once).
...
ensure_agent_detection() {
  ...
  src="${HERDR_PLUGIN_ROOT}/config/agent-detection/cmd.toml"
